...
// TIP: a fully functional Mac version of the tilemap project is provided as the
// Orthogonal-Tilemap Kobold2D template project.
#if KK_PLATFORM_IOS
...
-(void) draw
{
	...
	int numObjects = (int)objectLayer.objects.count;
	for (int i = 0; i < numObjects; i++)
	{
		NSDictionary* properties = [objectLayer.objects objectAtIndex:i];
		CGRect rect = [self getRectFromObjectProperties:properties tileMap:tileMap];
