...
-(void) macCheckTransitionToMenuScene:(ccTime)delta;
@end
...
-(id) init
{
	if ((self = [super init]))
	{
		...
		[self schedule:@selector(macCheckTransitionToMenuScene:)];
#endif
		...
		CCLabelTTF* label = [CCLabelTTF labelWithString:@"Touch Screen For Awesome" fontName:@"Times New Roman" fontSize:40];
		...
		CCLabelTTF* labelAligned = [CCLabelTTF labelWithString:@"I'm topright aligned!" fontName:@"Marker Felt" fontSize:30];
...
-(void) macCheckTransitionToMenuScene:(ccTime)delta
{
	// detect scene transition for Mac builds
	KKInput* input = [KKInput sharedInput];
	if ([input isAnyMouseButtonDownThisFrame] || [input isAnyKeyDownThisFrame])
	{
		LOG_EXPR(input.mouseLocation);
		CCScene* scene = [MenuScene scene];
		CCTransitionFade* transitionScene = [CCTransitionFade transitionWithDuration:3 scene:scene withColor:ccBLUE];
...
-(void) createLabelWithOffset:(int)offset
{
	...
	CCLabelTTF* label = [CCLabelTTF labelWithString:@"Hello Kobold2D" fontName:@"AppleGothic" fontSize:60];
