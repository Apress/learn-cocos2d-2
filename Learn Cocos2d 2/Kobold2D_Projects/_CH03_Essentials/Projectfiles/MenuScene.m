...
-(void) createMenu:(ccTime)delta
{
	...
	CCMenuItemToggle* item4 = [CCMenuItemToggle itemWithItems:items
		...
		NSUInteger index = [toggleItem selectedIndex];
		CCLOG(@"item 4 touched with block: %@ - selected index: %u", sender, (unsigned int)index);
	}];
...
-(void) menuItem3Touched:(id)sender
{
	...
	CCLOG(@"item 3 touched: %@ - selected index: %u", sender, (unsigned int)index);
}
